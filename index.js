...
const getStats = async (id, date) => {
	...
	sandwiches_chicken_ter = sandwiches_chicken_ter + getCount(stats.filter(item => item.product_name == "Сэндвич КУРИЦА ТЕРИЯКИ"))
	sandwiches_fish = sandwiches_fish + getCount(stats.filter(item => item.product_name == "Сэндвич РЫБА"))
	sandwiches_peperoni = sandwiches_peperoni + getCount(stats.filter(item => item.product_name == "Сэндвич ПЕПЕРОНИ"))
	sandwiches_egg = sandwiches_egg + getCount(stats.filter(item => item.product_name == "Сэндвич ЯЙЦО И ВЕТЧИНА"))
	sandwiches_tuna = sandwiches_tuna + getCount(stats.filter(item => item.product_name == "Сэндвич С ТУНЦОМ"))
